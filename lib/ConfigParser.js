...
class ConfigParser {
   ...
   */
    ...
    /**
     * @param {Object} #widget - the parsed xml document.
     */
    #widget;
    ...
    constructor (platform, path) {
      ...
      if (this.#rawDoc.widget?.['@_xmlns:cdv'] !== CDV_XMLNS_URI) {
          throw new Error(`The provided document does not include the proper xml namespace for xmlns:cdv. Expecting "${CDV_XMLNS_URI}".`);
      }

      // Grab only the platform specific data. Only accept the first element that matches. There should be only one.
      this.#platformData = this.#rawDoc.widget?.platform?.find(i => i['@_name'] === this.#platform) ?? {};

      this.#platformData = JSON.parse(JSON.stringify(this.#platformData));
      this.#root = JSON.parse(JSON.stringify(this.#rawDoc));

      // Update preference value if exisiting else add new.
      for (const platformPref of this.#platformData.preference) {
        const foundPref = this.#root.widget.preference.find(pref => pref['@_name'] === platformPref['@_name']);

        if (foundPref) {
          foundPref['@_value'] = platformPref['@_value'];
        } else {
          // Maybe add verbose?
          this.#root.widget.preference.push(platformPref);
        }
      }
      ...
      delete this.#root.widget.platform;

      this.#root = deepMerge(this.#root, { widget: { ...this.#platformData } });
      ...
      // console.log(this.#root);
      // console.log(this.#root.widget.preference);
    ...
    #setPreferences () {
      const unformattedData = this.#widget?.preference ?? [];

      for (const { '@_name': name, '@_value': value } of unformattedData) {
    ...
    #setPreference (nodeKey, value) {
      this.#preferences ||= {};
      this.#preferences[nodeKey] = value;
      return this;
    }

    getPreferences () {
        return this.#preferences ??= this.#setPreferences();
    }

    getPreference(key) {
        return this.#preferences ??= this.#setPreferences()[key];
    }

    getName() {
      return this.#widget?.name['#text'] ?? null;
    }

    setName(name) {
      this.#createIfMissing('name')
      this.#widget.name['#text'] = name;
      return this;
    }

    getShortName() {
      return this.#getTextSafe(this.#widget?.name['@_short'] ?? null);
    }

    setShortName(shortname) {
      this.#createIfMissing('name')
      this.#widget.name['@_short'] = shortname;
      return this;
    }
    ...
    getDescription () {
      return this.#getTextSafe(this.#widget?.description['#text'] ?? null);
    }

    setDescription (description) {
      this.#createIfMissing('name')
      this.#widget.description['#text'] = description;
      return this;
    }

    getVersion () {
      return this.#root['@_version'];
    }

    setVersion (value) {
      this.#root['@_version'] = value;
      return this;
    }

    getAuthor () {
      return this.#getTextSafe(this.#widget?.author['#text'] ?? null);
    }

    #createIfMissing(nodeKey) {
      this.#widget[nodeKey] ??= { '#text': '' };
      return this;
    }
