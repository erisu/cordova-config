...
class ConfigParser {
   ...
   */
    ...
    constructor (platform, path) {
      ...
      this.#doc = deepMerge(this.#doc, { widget: { ...this.#platformData } });
    ...
    #setPreferences () {
      const unformattedData = this.#doc.widget?.preference ?? [];
      this.#preferences ||= {};
    ...
    getName() {
      return this.#getTextSafe(this.#getAttribute('name', '#text'));
    }
    ...
    getShortName() {
      return this.#getTextSafe(this.#getAttribute('name', '@_short') ?? null);
    }

    setShortName(shortname) {
      return this.#findOrCreate('name')
        .#setAttribute('name', '@_short', shortname);
    }

    #findOrCreate(nodeKey) {
      this.#doc.widget[nodeKey] ??= { '#text': '' };
      return this;
    }

    #setText(node, text) {
      this.#setAttribute(node, '#text', text);
      return this;
    }

    #setAttribute(node, attrib, value) {
      this.#doc.widget[node][attrib] = value;
      return this;
    }

    #getAttribute(node, attrib) {
      return this.#doc.widget[node][attrib] ?? null;
    }
