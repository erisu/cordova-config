...
class ConfigParser {
   ...
   */
    ...
    constructor (platform, path) {
      ...
      this.#platformData.preference.forEach(platformPref => {
        let hasPref = false;

        for (let i = 0; i < this.#doc.widget.preference.length; i++) {
          const targetPref = this.#doc.widget.preference[i];

          if (targetPref['@_name'] === platformPref['@_name']) {
            // Maybe add verbose?
            targetPref['@_value'] = platformPref['@_value'];
            hasPref = true;
            return;
          }
        }
